import { DocumentDuplicateIcon, CodeBracketIcon, ArrowDownTrayIcon } from "@heroicons/react/24/outline";
    ...
    <div className="side-bar flex flex-col align-center justify-between h-full p-1 bg-slate-200 transition dark:bg-slate-800">
      <ol className="relative">
        <SidebarLink to={`/j/${doc.id}`} hotKey="option+1,alt+1">
          ...
          <DocumentDuplicateIcon className="p-2 w-full h-full" />
        </SidebarLink>
      ...
      <ol>
        <SidebarLink>
          <a href={`/api/docs/${doc.id}/raw`} download={`${doc.id}.json`}>
            <ToolTip arrow="left">
              <Body>Download</Body>
            </ToolTip>
            <ArrowDownTrayIcon className="p-2 w-full h-full" />
          </a>
        </SidebarLink>
      </ol>
    </div>
