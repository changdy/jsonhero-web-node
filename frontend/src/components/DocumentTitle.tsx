...
import { updateDocTitle } from "~/api/docs";
...
export function DocumentTitle() {
  ...
  const handleSave = async () => {
    if (editedTitle === doc.title) return;
    setIsSubmitting(true);
    try {
      await updateDocTitle(doc.id, editedTitle);
      ref.current?.blur();
    } catch {
      // Handle error
    } finally {
      ...
      <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
        ...
        >
          <label className="relative block group">
            <PencilSquareIcon className="h-5 w-5 absolute top-1/2 transform -translate-y-1/2 left-3 text-white opacity-0 transition pointer-events-none group-hover:opacity-80 group-focus:opacity-80" />
            <input
              ref={ref}
              className={
