...
export type PreviewJsonType = {
  url: string;
  contentType: "json";
  json: unknown;
};

export type PreviewInfo = PreviewImageType | PreviewHtmlType | PreviewJsonType;
export type PreviewResult = PreviewInfo | { error: string };

type YouTubeLinkDetails = {
  type: "youtube";
  videoId: string;
  duration: string;
  viewCount: number;
